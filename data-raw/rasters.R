...
ids <- c("akcan2km", "akcan1km", "ak1km", "swveg", "swfmoBuffer", "swfmoRatios")
files <- file.path(
...
proj4 <- "+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs"
purrr::walk2(ids, files, ~assign(.x, readAll(raster(.y)), envir = .GlobalEnv))
for(i in ids){
  x <- get(i)
  if(i %in% c("akcan2km", "akcan1km", "ak1km")) x[!is.na(x)] <- 1L
  projection(x) <- proj4
  assign(i, x)
}
purrr::map(ids, ~(projection(get(.x)))) # check all AK Albers
usethis::use_data(akcan2km, akcan1km, ak1km, swveg, swfmoBuffer, swfmoRatios, compress="xz")
