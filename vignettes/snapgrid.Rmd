...
This introduction is only intended to expose you to some data sets in `snapgrid`, not to introduce the `raster` package. There is simply far to much to include here anyway. However, the `raster` package is highly applicable to since `snapgrid` contains purely raster data. To learn more about how to efficiently and effectively work with this type of data in R, see the documentation associated with the `raster` package.
